...
def main_app(): #
    ...
    st.markdown("---") #
    st.header("🔬 Chế Độ Dự Đoán & Grad-CAM") #
    
    # Sử dụng cột để bố trí ảnh và kết quả/Grad-CAM
    ...
    with col_display1:
        ...
        if prediction_mode == "Tải Ảnh Lên": #
            ...
            if st.button("Lấy Ảnh Ngẫu Nhiên", key="get_random_image_button"):
                if not os.path.isdir(st.session_state.test_dir_input): #
                    ...
                    if not image_files: #
                        ...
                        try:
                            ...
                            caption_image_source = f"Ảnh Ngẫu Nhiên: {os.path.basename(random_image_path)}"
                            st.session_state.current_image_pil = image_pil_for_prediction # Lưu lại để hiển thị lại nếu cần
                            st.session_state.current_caption = caption_image_source
                        except Exception as e: #
        ...
        if image_pil_for_prediction:
            if st.button("Thực Hiện Dự Đoán & Hiện Grad-CAM", key="predict_and_gradcam_button"):
            ...
            if prediction_mode == "Tải Ảnh Lên":
                 st.info("Vui lòng tải ảnh lên để dự đoán.")
            # Không cần thông báo cho chế độ ảnh ngẫu nhiên vì đã có nút "Lấy Ảnh Ngẫu Nhiên"
    ...
    with col_display2:
        ...
        if last_prediction_info and last_prediction_info.get("image_pil") is not None: #
            ...
            if not (0 <= target_category_for_gradcam < num_classes_loaded): #
                ...
                # Không cần nút "Tạo Grad-CAM", sẽ tự động tạo khi input thay đổi hoặc có dự đoán mới
            ...
            # Trường hợp người dùng đã nhấn nút dự đoán nhưng chưa có ảnh hợp lệ
             st.info("Vui lòng cung cấp ảnh và nhấn 'Thực Hiện Dự Đoán & Hiện Grad-CAM' để xem kết quả và Grad-CAM.")
        else:
            st.info("Kết quả dự đoán và Grad-CAM sẽ được hiển thị ở đây sau khi bạn thực hiện dự đoán.")
